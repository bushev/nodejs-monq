...
function Queue(connection, name, options) {
    ...
    this.connection.getDb(function(err, db) {
...
Queue.prototype.getCollection = function (callback) {
    var self = this;
...
Queue.prototype.enqueue = function (name, params, options, callback) {
    ...
    this.collection.findOne({ name: name, params: params, queue: this.name }, function(err, data) {
        if (err) return callback(err);

        if (data) {

            // Do not add duplicated jobs
            callback();
